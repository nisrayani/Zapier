...
async function main() {
    ...
    console.log(`Connected to Kafka broker from worker and ready to consume from topic ${KAFKA_TOPIC}`);
    const producer = kafka.producer();
    await producer.connect();
    console.log(`Connected to Kafka broker from worker and ready to publish to topic ${KAFKA_TOPIC}`);
    await consumer.run({
        autoCommit: false,
        eachMessage: async ({ topic, partition, message }) => {
            ...
            const currentAction = actions?.find((action) => action.sortingOrder == stage);
            if (!currentAction) {
            ...
            switch (currentAction.type.name) {
                case "email":
                    ...
                    const body = parse(currentAction.metadata.body, zapRunDetailsMetaData); // You just received {comment.amount}
                    const to = parse(currentAction.metadata.email, zapRunDetailsMetaData); // {comment.email}
                    sendEmail(to, body);
                    break;
                case "solana_send":
                    console.log("Processing Solana send");
                    const amount = parse(currentAction.metadata.amount, zapRunDetailsMetaData);
                    const address = parse(currentAction.metadata.address, zapRunDetailsMetaData);
            ...
            if (stage !== lastStage) {
                ...
                console.log(`Pushed above event ${parsedValue} with stage: ${currentAction.sortingOrder + 1} to Kafka`);
            }
            else {
